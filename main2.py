...
# --- SECURITY CONFIG ---
SECRET_KEY = "SGP_PROJECT_SECRET_KEY" # Change this in production
ALGORITHM = "HS256"
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if not user or form_data.password != user["password"]: # Plain pass for now as requested
        raise HTTPException(status_code=400, detail="Incorrect username or password")
    
    access_token = create_access_token(data={"sub": user["username"], "role": user["role"]})
    return {"access_token": access_token, "token_type": "bearer", "role": user["role"]}

# --- 1. ADMIN: ADD FEST (Protected) ---
@app.post("/admin/add-fest")
async def admin_add_fest(fest: FestInput, current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "admin":
        raise HTTPException(status_code=403, detail="Only Admins can add fests")
    
    fest_data = {"name": fest.event_name, "date": str(fest.event_date)}
    fest_collection.insert_one(fest_data)
    return {"message": "Fest added by Admin"}
